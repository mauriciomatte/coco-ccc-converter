...
export default function App() {
  ...
  // Rede de segurança: qualquer término/cancelamento de arraste (ESC, soltar fora, linha desmontada)
  // limpa o item arrastado — senão a cópia do disco em srcBuffer ficaria retida na memória.
  useEffect(() => {
    const clear = () => { dskDragItem.current = null; };
  ...
  const [os9Doc, setOs9Doc] = useState<Os9Doc | null>(null); // documento da ABA OS-9 (RBF): leitura (filePath+base) ou edição (buffer)
  const [os9Dirty, setOs9Dirty] = useState<boolean>(false); // a aba OS-9 tem edição não salva?
  const [os9PendingExternal, setOs9PendingExternal] = useState<Os9Doc | null>(null); // doc externo aguardando confirmação (edição não salva)
  ...
  // Erros do arraste-para-fora (startDrag) chegam aqui para aparecer no console de diagnóstico.
  useEffect(() => {
    if (!window.cocoApi || typeof window.cocoApi.onDragError !== 'function') return;
    const off = window.cocoApi.onDragError((msg: string) => addLog(`Arrastar p/ Explorer: ${msg}`, `Drag to Explorer: ${msg}`, 'error'));
    return () => { if (typeof off === 'function') off(); };
  }, []);
      ...
      try {
        if (window.cocoApi && typeof window.cocoApi.loadConfig === 'function') {
          ...
          if (s) {
            ...
            if (s.paneNewFormat && s.paneNewFormat.A && s.paneNewFormat.B) setPaneNewFormat(s.paneNewFormat);
  ...
  // Abre um documento na ABA OS-9. Se a aba tem edição NÃO salva, pede confirmação antes de substituir
  // (senão a edição em memória seria perdida silenciosamente).
  const openOs9Doc = (doc: Os9Doc) => {
    if (os9Dirty) { setOs9PendingExternal(doc); return; }
  ...
  // Desfragmentação de disco Dragon INTEIRA (sem a animação por-grânulo do RS-DOS — o Dragon usa
  // bitmap+SAB, não FAT de grânulos). Reescreve os arquivos contíguos e regrava bitmap/diretório.
  const handleDragonDefrag = async (which: 'A' | 'B') => {
    const pane = getPane(which);
    if (!pane || !pane.files.length) { addLog('Sem arquivos para desfragmentar.', 'No files to defragment.', 'warn'); return; }
    if (typeof window.cocoApi.dskDefragDragon !== 'function') {
      addLog('Reinicie o app: a desfragmentação Dragon (preload) não está carregada.', 'Restart the app: Dragon defrag (preload) is not loaded.', 'error');
      return;
    }
    const before = pane.files.filter((f: any) => f.fragmented).length;
    try {
      setImageBusy(true);
      pushDskUndo();
      const res = await window.cocoApi.dskDefragDragon(pane.buffer);
      if (!res.success) { addLog(`Desfragmentar (Dragon): ${res.error}`, `Defrag (Dragon): ${res.error}`, 'error'); return; }
      if (selectedDsk?.pane === which) setSelectedDsk(null);
      await refreshPane(which, res.image);
      const after = await window.cocoApi.readDskDirectory(res.image);
      const afterFrag = after.success ? after.files.filter((f: any) => f.fragmented).length : 0;
      addLog(`Disco Dragon do painel ${which} desfragmentado (${before}→${afterFrag} arquivo(s) fragmentado(s)). Lembre-se de salvar.`,
        `Pane ${which} Dragon disk defragmented (${before}→${afterFrag} fragmented file(s)). Remember to save.`, 'success');
    } catch (err: any) { addLog(`Desfragmentar (Dragon): ${err.message}`, `Defrag (Dragon): ${err.message}`, 'error'); }
  ...
  const loadPaneFromBuffer = async (which: 'A' | 'B', full: Uint8Array, fileName: string, index = 0, sourcePath?: string, created = false): Promise<boolean> => {
    ...
    // O formato de "Novo disco" deste painel se auto-ajusta ao disco carregado (35/40T, CoCo/Dragon).
    setPaneNewFormat(p => ({ ...p, [which]: paneFormatKey(res.format, slice.length) }));
  ...
  // Troca o disco ativo de um contêiner. Se há edição NÃO salva num slot MiniIDE, NÃO grava sozinho
  // no .img — abre um modal para o usuário decidir (cancelar / trocar de mídia / Salvar Como / gravar).
  ...
  // Executa a troca de fato — em memória (DriveWire) fatia o buffer; em arquivo (MiniIDE/CoCoSDC)
  // RELÊ só aquele disco do .img sob demanda (sem recarregar a imagem inteira).
  const doSelectContainerDisk = async (which: 'A' | 'B', index: number) => {
    const pane = getPane(which);
    if (!pane || !pane.container) return;
    const c = pane.container;
  ...
  const startDefragAnimation = async (which: 'A' | 'B', order: 'dir' | 'alpha' | 'size') => {
    // Dragon não tem FAT de grânulos (a animação por-grânulo não se aplica): desfragmenta o disco inteiro.
    if (isDragonPane(which)) { await handleDragonDefrag(which); return; }
    const pane = getPane(which);
  ...
  const handleDefragFile = async (which: 'A' | 'B', entry: any) => {
    // Dragon: a desfragmentação é por DISCO (sem realocação por-arquivo) → roda a do disco inteiro.
    if (isDragonPane(which)) { await handleDragonDefrag(which); return; }
    const pane = getPane(which);
  ...
  const doAddBatch = async (which: 'A' | 'B', files: any[], mode: 'add' | 'overwrite' | 'rename') => {
    ...
    if (pane) {
      ...
      // Painel vazio: cria uma imagem nova NO FORMATO ESCOLHIDO DESTE painel (CoCo 35/40 ou Dragon),
      // em vez de um padrão fixo — assim o drop num painel vazio respeita o formato que o usuário definiu.
      ...
      addLog(`Imagem nova (${lbl}) criada no painel ${which} para receber o(s) arquivo(s).`, `New ${lbl} image created in pane ${which} to receive the file(s).`, 'info');
    }
    let added = 0;
    try {
  ...
  // Confirmado: monta o disco do painel no drive 0. `reset=true` faz hard reset (boot limpo → teste
  // limpo, sem resíduo); `reset=false` monta o disco SEM resetar (mantém o que está rodando).
  const proceedTestInXroar = (reset: boolean) => {
  ...
  // O resultado fica no painel marcado como NÃO salvo → o "Salvar" grava (em MiniIDE, regrava o slot
  // preservando o nome SIDEKICK do drive, que restauramos aqui a partir do LSN 322 / offset 82.432).
  const handleFormatPane = async (which: 'A' | 'B', mode: 'quick' | 'full') => {
    setFormatConfirm(null);
    const pane = getPane(which);
    if (!pane) return;
    pushDskUndo();
    try {
      const res = await window.cocoApi.dskFormat(pane.buffer, mode);
      if (!res.success) { addLog(`Formatar: ${res.error}`, `Format: ${res.error}`, 'error'); return; }
      const formatted = new Uint8Array(res.image);
      // MiniIDE: preserva o nome do drive do SIDEKICK (setor LSN 322 = offset de-doubled 82.432).
      const NAME_OFF = 322 * 256; // 82.432
      if (pane.container?.kind === 'miniide' && pane.buffer.length >= NAME_OFF + 256 && formatted.length >= NAME_OFF + 256) {
        formatted.set(pane.buffer.subarray(NAME_OFF, NAME_OFF + 256), NAME_OFF);
      }
      if (selectedDsk?.pane === which) setSelectedDsk(null);
      await refreshPane(which, formatted);
      markDirty(which);
      addLog(`Painel ${which} FORMATADO (${mode === 'full' ? 'completa' : 'rápida'}). Salve para gravar no disco/imagem.`,
             `Pane ${which} FORMATTED (${mode === 'full' ? 'full' : 'quick'}). Save to write it to the disk/image.`, 'warn');
    } catch (err: any) { addLog(`Formatar: ${err.message}`, `Format: ${err.message}`, 'error'); }
  };

  // INSERIR uma imagem .dsk (35T RS-DOS) num slot — pensado para slots VAZIOS da MiniIDE. Carrega o
  // disco escolhido no painel e marca como não salvo; o "Salvar" grava no slot (com sector-doubling).
  const handleInsertImage = async (which: 'A' | 'B') => {
    const pane = getPane(which);
    if (!pane?.container || pane.container.kind !== 'miniide') {
      addLog('Inserir imagem: disponível só em slot de contêiner MiniIDE.', 'Insert image: only available on a MiniIDE container slot.', 'warn');
      return;
    }
    try {
      const res = await window.cocoApi.pickDiskImage();
      if (res?.cancelled) return;
      if (!res?.success) { addLog(`Inserir imagem: ${res?.error}`, `Insert image: ${res?.error}`, 'error'); return; }
      if (!res.valid35tRsdos) {
        addLog(`Inserir imagem: precisa ser um disco RS-DOS de 35 trilhas (161.280 B). Este tem ${res.size} B ${res.size !== 161280 ? '(tamanho incompatível com o slot)' : '(FAT não-RS-DOS)'}.`,
               `Insert image: must be a 35-track RS-DOS disk (161,280 B). This one is ${res.size} B ${res.size !== 161280 ? '(size incompatible with the slot)' : '(non-RS-DOS FAT)'}.`, 'error');
        return;
      }
      pushDskUndo();
      const slotN = pane.container.entries?.[pane.container.index]?.slot ?? pane.container.index;
      if (selectedDsk?.pane === which) setSelectedDsk(null);
      await refreshPane(which, new Uint8Array(res.data));
      markDirty(which);
      addLog(`Imagem "${res.name}" inserida no slot ${slotN} (painel ${which}). Salve para gravar no .img (faça numa CÓPIA antes do cartão real).`,
             `Image "${res.name}" inserted into slot ${slotN} (pane ${which}). Save to write it to the .img (do it on a COPY before the real card).`, 'success');
  ...
  const handleRenameDrive = async (which: 'A' | 'B', name: string) => {
    setRenameDrive(null);
    const pane = getPane(which);
    if (!pane?.container || pane.container.kind !== 'miniide') return;
    if (pane.emptySlot || pane.diskUnreadable) { addLog('Nomear drive: navegue até um disco ocupado.', 'Name drive: go to an occupied disk.', 'warn'); return; }
    try {
      pushDskUndo();
  ...
  const doDskNew = async (which: 'A' | 'B') => {
    ...
    try {
      ...
      addLog(
        ...
        `New ${label} disk created in pane ${which}.`,
        'success');
    } catch (err: any) { addLog(`New disk: ${err.message}`, `New disk: ${err.message}`, 'error'); }
  };

  // Troca o formato de "Novo disco" de um painel. Se o painel já tem arquivos, decide o que fazer:
  //  • painel vazio → só muda a preferência;
  //  • mesma família RS-DOS, geometria diferente (35↔40) → pergunta e RECRIA mantendo os arquivos;
  ...
  const handlePaneFormatChange = (which: 'A' | 'B', newFmt: PaneFmt) => {
    ...
    const curDragon = pane!.format === 'dragon';
    const newDragon = newFmt === 'dragon';
    if (curDragon !== newDragon) {
      addLog('Trocar entre CoCo e Dragon muda o CONTEÚDO dos arquivos (tokens/cabeçalhos) — use o conversor "→ Dragon" por arquivo. O seletor não foi alterado.',
             'Switching between CoCo and Dragon changes file CONTENT (tokens/headers) — use the per-file "→ Dragon" converter. The selector was not changed.', 'warn');
      return; // não muda o seletor (revert): ele reflete o disco aberto
    }
    if (newDragon) return; // Dragon só tem 40T por ora
    // RS-DOS 35↔40 com arquivos → confirma a recriação preservando arquivos
    setReformatConfirm({ which, newFmt: newFmt as 'coco35' | 'coco40', count: pane!.files.length });
  };

  // Recria a imagem do painel no novo formato RS-DOS (35↔40) MANTENDO os arquivos. Aborta sem alterar
  // se algum arquivo não couber (ex.: 40T→35T com dados demais).
  const reformatPaneKeepingFiles = async (which: 'A' | 'B', newFmt: 'coco35' | 'coco40') => {
    setReformatConfirm(null);
    const pane = getPane(which);
    if (!pane) return;
    try {
      const dir = await window.cocoApi.readDskDirectory(pane.buffer);
      if (!dir.success) { addLog(`Reformatar: ${dir.error}`, `Reformat: ${dir.error}`, 'error'); return; }
      const blank = await window.cocoApi.dskNewBlank(newFmt === 'coco40' ? 40 : 35);
      if (!blank.success) { addLog(`Reformatar: ${blank.error}`, `Reformat: ${blank.error}`, 'error'); return; }
      let buffer = new Uint8Array(blank.image);
      let added = 0;
      for (const entry of dir.files) {
        const ex = await window.cocoApi.dskExtractRaw(pane.buffer, entry);
        if (!ex.success) { addLog(`Reformatar: extração falhou (${entry.fullName}) — imagem mantida.`, `Reformat: extract failed (${entry.fullName}) — image kept.`, 'error'); return; }
        const r = await window.cocoApi.dskAddBytes(buffer, entry.name, entry.ext, entry.fileType, entry.asciiFlag || 0, ex.data);
        if (!r.success) { addLog(`Reformatar ABORTADO: "${entry.fullName}" não cabe no formato ${newFmt === 'coco40' ? '40T' : '35T'} (${r.error}). Imagem mantida.`, `Reformat ABORTED: "${entry.fullName}" doesn't fit ${newFmt === 'coco40' ? '40T' : '35T'} (${r.error}). Image kept.`, 'error'); return; }
        buffer = r.image; added++;
      }
      pushDskUndo();
      const base = (pane.fileName || 'DISCO').replace(/\.(dsk|vdk)$/i, '');
      await loadPaneFromBuffer(which, buffer, `${base}.dsk`);
      markDirty(which);
      addLog(`Painel ${which} recriado em ${newFmt === 'coco40' ? '40T (JDOS)' : '35T'} mantendo ${added} arquivo(s). Lembre-se de salvar.`,
             `Pane ${which} re-created as ${newFmt === 'coco40' ? '40T (JDOS)' : '35T'} keeping ${added} file(s). Remember to save.`, 'success');
    } catch (err: any) { addLog(`Reformatar: ${err.message}`, `Reformat: ${err.message}`, 'error'); }
  };
  ...
  // Remove arquivos (por nome) da imagem de um painel — usado pelo MOVER (Shift+arrastar).
  const deleteEntriesFromPane = async (which: 'A' | 'B', entries: any[]) => {
    const pane = getPane(which);
    if (!pane || !entries.length) return;
    let buffer = pane.buffer;
    for (const entry of entries) {
      const dir = await window.cocoApi.readDskDirectory(buffer);   // re-acha por nome (índices mudam a cada delete)
      if (!dir.success) break;
      const cur = dir.files.find((f: any) => f.fullName === entry.fullName);
      if (!cur) continue;
      const del = await window.cocoApi.dskDeleteFile(buffer, cur);
      if (del.success) buffer = del.image;
    }
    await refreshPane(which, buffer);
  };
  ...
  const handleDskInternalDrop = async (targetWhich: 'A' | 'B', source: { pane: 'A' | 'B'; entries: any[]; srcBuffer?: Uint8Array }, move = false) => {
    ...
    try {
      const files: any[] = [];
      const moved: any[] = []; // entradas extraídas com sucesso (candidatas a remover da origem no MOVER)
      ...
      if (files.length) {
        ...
        // MOVER: os dados já estão em memória (files), então é seguro remover da origem.
        if (move && moved.length) {
          await deleteEntriesFromPane(source.pane, moved);
          addLog(`${moved.length} arquivo(s) movido(s) do painel ${source.pane} para ${targetWhich}.`,
                 `${moved.length} file(s) moved from pane ${source.pane} to ${targetWhich}.`, 'success');
        }
  ...
  const handlePaneDrop = (which: 'A' | 'B', e: React.DragEvent) => {
    ...
    // Shift (sem Ctrl) = MOVER (remove da origem); senão COPIAR. Igual à convenção do Windows.
    const move = e.shiftKey && !e.ctrlKey;
    if (item) handleDskInternalDrop(which, item, move);
  };
  ...
  const renderDskPane = (which: 'A' | 'B', pane: any) => {
    ...
    return (
      <div
        ...
        onDragOver={(e) => {
          e.preventDefault();
          // Convenção do Windows: Ctrl=copiar, Shift=mover; padrão=copiar. O dropEffect muda o
          // cursor do SO (mostra o "+" no copiar, a seta no mover) durante o arraste interno.
          if (dskDragItem.current) e.dataTransfer.dropEffect = (e.shiftKey && !e.ctrlKey) ? 'move' : 'copy';
        }}
                ...
                <div className="flex flex-col gap-2 items-center text-center w-full">
                  ...
                  {pane.container && (
                    <div className="flex flex-col gap-1.5 bg-slate-950/40 rounded p-2 border border-[#a78bfa]/40 w-full" style={{ maxWidth: 172 }} onClick={(e) => e.stopPropagation()}>
                      ...
                      {pane.container.kind === 'miniide' && !pane.emptySlot && !pane.diskUnreadable && (
                        <button
          ...
          <div className="flex-1 overflow-y-auto" style={{ minHeight: 0 }}>
            {pane && pane.diskUnreadable ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-3 p-6 text-center">
                ...
                <span className="text-[11px] text-[var(--text-secondary)] leading-relaxed max-w-[420px]">
                  {currentLang === 'pt-br'
                    ? 'Este slot (000–255) não contém disco. Insira uma imagem .dsk (35T RS-DOS) aqui, ou formate-o. Use ◀ ▶ para navegar pelos slots.'
                    : 'This slot (000–255) holds no disk. Insert a .dsk image (35T RS-DOS) here, or format it. Use ◀ ▶ to browse slots.'}
                </span>
                  ...
                  {pane.files.map((f: any, idx: number) => (
                    <tr
                      ...
                      onDragStart={(e) => {
                        ...
                        e.dataTransfer.effectAllowed = 'copyMove'; // permite copiar (Ctrl) e mover (Shift)
                      }}
                    ...
                    >
                      <td className="p-2 font-mono">
                        {/* Alça de arraste-para-fora: drag NATIVO do SO → soltar o arquivo no Windows Explorer.
                            (O resto da linha continua sendo o arraste interno A→B.) stopPropagation evita o
                            onDragStart do <tr>; preventDefault cancela o DnD HTML5 e deixa o startDrag nativo agir. */}
                        <span
                          draggable
                          ...
                          onDragStart={(e) => {
                            // Sem isto, o Chromium dispara o arraste da <tr> (interno), não o da alça. Desativamos
                            // o draggable da <tr> no mousedown → o arraste parte da ALÇA; restauramos logo depois.
                            e.stopPropagation();
                            e.preventDefault();
                            const tr = (e.currentTarget as HTMLElement).closest('tr');
                            window.cocoApi.startFileDrag?.(pane.buffer, f, f.fullName);
                            if (tr) setTimeout(() => tr.setAttribute('draggable', 'true'), 0);
                          }}
                          onClick={(e) => e.stopPropagation()}
                          onDoubleClick={(e) => e.stopPropagation()}
                          title={t('dskDragOutHint')}
                          className="cursor-grab mr-1 select-none text-[var(--text-muted)] hover:text-[var(--primary)]"
                          style={{ display: 'inline-block', padding: '0 3px', fontSize: 13 }}
                        >⠿</span>
                        {f.name}
                      </td>
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'nowrap', justifyContent: 'center', flexShrink: 0 }}>
                    <button
                      onClick={() => { if (isDragon) { handleDragonDefrag(which); } else { setDefragOrder('dir'); setDefragModal({ which }); } }}
                      disabled={imageBusy || !pane.files.length}
                      className="dsk-tool" style={{ padding: '3px 8px', fontSize: 10, whiteSpace: 'nowrap' }}
                      title={isDragon ? (currentLang === 'pt-br' ? 'Desfragmentar o disco Dragon inteiro (reescreve os arquivos contíguos + regrava bitmap/diretório)' : 'Defragment the whole Dragon disk (rewrite files contiguous + rebuild bitmap/directory)') : (currentLang === 'pt-br' ? 'Desfragmentar o disco inteiro (reescreve numa imagem nova, contígua)' : 'Defragment the whole disk (rewrite to a fresh contiguous image)')}
                    >DEFRAG</button>
                    <button
                      onClick={() => sel && handleDefragFile(which, sel)}
                      disabled={imageBusy || !selFrag || isDragon}
                      className="dsk-tool" style={{ padding: '3px 8px', fontSize: 10, whiteSpace: 'nowrap' }}
                      title={isDragon ? (currentLang === 'pt-br' ? 'No Dragon a desfragmentação é por DISCO inteiro — use o botão DEFRAG' : 'On Dragon, defragmentation is whole-disk — use the DEFRAG button') : (currentLang === 'pt-br' ? 'Desfragmentar apenas o arquivo selecionado (precisa de um vão contíguo livre)' : 'Defragment only the selected file (needs a contiguous free run)')}
  ...
  return (
    <div className="app-container">
      ...
      <header className="app-header">
        ...
        {/* Tab bar */}
        <div className="flex items-center bg-slate-900/40 border border-[var(--border)] p-1 rounded-xl backdrop-blur-md" style={{ gap: 3, flexShrink: 0 }}>
          <button
          ...
          <div className="flex-1 flex flex-col overflow-hidden p-3" style={{ minHeight: 0 }}>
            ...
            <div className="flex items-center gap-1.5 mb-3 flex-wrap flex-shrink-0">
              ...
              {/* Ações de ARQUIVO — CONTEXTUAIS: aparecem só quando há arquivo(s) selecionado(s).
                  (Sem botão Recortar: mover é por arrastar com Shift; copiar com Ctrl.) */}
              {!!selectedDsk?.entries.length && (
                <>
                  <div className="w-[1px] h-5 bg-[var(--border)] mx-1" />
              ...
              {/* Colar — só quando há algo na área de transferência. */}
              {dskClipboard && (
                <>
                  <div className="w-[1px] h-5 bg-[var(--border)] mx-1" />
                  <button onClick={handleDskPaste} title={t('dskToolPaste')} aria-label={t('dskToolPaste')} className="dsk-tool"><Clipboard size={15} /></button>
                </>
              )}
              <div className="w-[1px] h-5 bg-[var(--border)] mx-1" />
              {paneA && <button onClick={handleCopyPaneAToB} title={t('dskToolCopyAtoB')} aria-label={t('dskToolCopyAtoB')} className="dsk-tool"><ArrowRight size={15} /></button>}
      ...
      {/* Aviso: abrir/criar outro disco OS-9 com edição não salva na aba (perderia as alterações). */}
      {os9PendingExternal && (
        <div className="glass-modal-overlay" onClick={() => setOs9PendingExternal(null)}>
          <div className="glass-panel p-5 flex flex-col gap-4" style={{ width: 480, maxWidth: '92%' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-amber-950/30 text-amber-400 flex-shrink-0"><AlertTriangle size={20} /></div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wide">{currentLang === 'pt-br' ? 'Alterações OS-9 não salvas' : 'Unsaved OS-9 changes'}</h3>
            </div>
            <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
              {currentLang === 'pt-br'
                ? 'A aba OS-9 tem alterações NÃO salvas. Abrir/criar outro disco vai descartá-las. Para salvar, cancele, vá à aba OS-9 e use "Salvar" / "Salvar Como".'
                : 'The OS-9 tab has UNSAVED changes. Opening/creating another disk will discard them. To save, cancel, go to the OS-9 tab and use "Save" / "Save As".'}
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setOs9PendingExternal(null)} className="btn btn-secondary py-2 px-4 text-xs font-bold uppercase">{t('modalCancel')}</button>
              <button onClick={() => { const d = os9PendingExternal; setOs9PendingExternal(null); setOs9Doc(d); setActiveTab('os9'); }} className="btn btn-primary py-2 px-4 text-xs font-bold uppercase">{currentLang === 'pt-br' ? 'Descartar e continuar' : 'Discard & continue'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="glass-modal-overlay" onClick={() => setImgWriteConfirm(null)}>
          <div className="glass-panel p-5 flex flex-col gap-4" style={{ width: 520, maxWidth: '93%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 pt-1 flex-wrap">
              ...
              {imgWriteConfirm.pendingSwitch != null && (
      ...
      {renameDrive && (
        <div className="glass-modal-overlay" onClick={() => setRenameDrive(null)}>
          <div className="glass-panel p-5 flex flex-col gap-3" style={{ width: 430, maxWidth: '92%' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-amber-950/30 text-amber-400 flex-shrink-0"><FileText size={20} /></div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wide">{currentLang === 'pt-br' ? 'Nome do drive (SIDEKICK)' : 'Drive name (SIDEKICK)'}</h3>
            </div>
            <input
              value={renameDrive.current} maxLength={8} autoFocus
              onChange={(e) => setRenameDrive(r => r ? { ...r, current: e.target.value.toUpperCase().replace(/[^\x20-\x7E]/g, '').slice(0, 8) } : r)}
              onKeyDown={(e) => { if (e.key === 'Enter' && renameDrive.current.trim()) handleRenameDrive(renameDrive.which, renameDrive.current); }}
              placeholder={currentLang === 'pt-br' ? 'NOME (até 8 caracteres)' : 'NAME (up to 8 chars)'}
              className="input-text py-2 px-3 text-sm font-mono uppercase tracking-wider"
            />
            {!renameDrive.named && (
              <p className="text-[11px] text-amber-300 leading-relaxed">
                {currentLang === 'pt-br'
                  ? '⚠ Este drive não tinha nome — o catálogo do SIDEKICK será CRIADO (recurso experimental). Faça numa CÓPIA e valide no hardware antes de confiar.'
      ...
      {formatConfirm && (
        <div className="glass-modal-overlay" onClick={() => setFormatConfirm(null)}>
          <div className="glass-panel p-5 flex flex-col gap-4" style={{ width: 480, maxWidth: '92%' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-rose-950/30 text-rose-400 flex-shrink-0"><Trash2 size={20} /></div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wide">{currentLang === 'pt-br' ? `Formatar Imagem do Painel ${formatConfirm.which}?` : `Format Pane ${formatConfirm.which} image?`}</h3>
            </div>
            <p className="text-xs text-rose-300 leading-relaxed font-bold">
              {currentLang === 'pt-br'
                ? '⚠ APAGA OS DADOS desta imagem. Em contêiner (MiniIDE), o disco/slot fica vazio ao salvar (o nome do drive do SIDEKICK é preservado). Faça isto numa CÓPIA antes de gravar no cartão real.'
                : '⚠ ERASES THE DATA of this image. In a container (MiniIDE), the disk/slot becomes empty on save (the SIDEKICK drive name is preserved). Do this on a COPY before writing to the real card.'}
            </p>
            <p className="text-[11px] text-[var(--text-secondary)] leading-relaxed">
              {currentLang === 'pt-br'
                ? 'RÁPIDA: limpa só o diretório e a FAT (referências) — instantânea, dados antigos ficam nos granules. COMPLETA: apaga a imagem inteira (tudo 0xFF).'
                : 'QUICK: clears only the directory and FAT (references) — instant, old data stays in the granules. FULL: wipes the whole image (all 0xFF).'}
            </p>
            <div className="flex justify-end gap-2 pt-1 flex-wrap">
              <button onClick={() => setFormatConfirm(null)} className="btn btn-secondary py-2 px-4 text-xs font-bold uppercase">{t('modalCancel')}</button>
              <button onClick={() => handleFormatPane(formatConfirm.which, 'quick')} className="btn btn-secondary py-2 px-4 text-xs font-bold uppercase">{currentLang === 'pt-br' ? 'Formatação Rápida' : 'Quick Format'}</button>
              <button onClick={() => handleFormatPane(formatConfirm.which, 'full')} className="btn btn-primary py-2 px-4 text-xs font-bold uppercase flex items-center gap-1.5"><Trash2 size={13} /> {currentLang === 'pt-br' ? 'Formatação Completa' : 'Full Format'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="glass-modal-overlay" onClick={() => setXroarTestConfirm(false)}>
          <div className="glass-panel p-5 flex flex-col gap-4" style={{ width: 430, maxWidth: '90%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 pt-1 flex-wrap">
              ...
              <button onClick={() => proceedTestInXroar(false)} className="btn btn-secondary py-2 px-4 text-xs font-bold uppercase flex items-center gap-1.5">
                <MonitorPlay size={13} /> {currentLang === 'pt-br' ? 'Testar sem reset' : 'Test without reset'}
              </button>
      ...
      {reformatConfirm && (
        <div className="glass-modal-overlay" onClick={() => setReformatConfirm(null)}>
          <div className="glass-panel p-5 flex flex-col gap-4" style={{ width: 460, maxWidth: '90%' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-amber-950/30 text-amber-400 flex-shrink-0"><AlertTriangle size={20} /></div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wide">{currentLang === 'pt-br' ? `Recriar imagem em ${reformatConfirm.newFmt === 'coco40' ? '40T (JDOS)' : '35T'}?` : `Re-create image as ${reformatConfirm.newFmt === 'coco40' ? '40T (JDOS)' : '35T'}?`}</h3>
            </div>
            <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
              {currentLang === 'pt-br'
                ? `O Painel ${reformatConfirm.which} vira um disco ${reformatConfirm.newFmt === 'coco40' ? '40 trilhas (JDOS/CODIMEX)' : '35 trilhas (DECB)'} MANTENDO os ${reformatConfirm.count} arquivo(s). Se algum arquivo não couber no novo formato, nada é alterado. (Operação não salva — use "Salvar" depois.)`
                : `Pane ${reformatConfirm.which} becomes a ${reformatConfirm.newFmt === 'coco40' ? '40-track (JDOS/CODIMEX)' : '35-track (DECB)'} disk KEEPING the ${reformatConfirm.count} file(s). If any file doesn't fit the new format, nothing changes. (Not saved — use "Save" afterwards.)`}
            </p>
            <div className="flex justify-end gap-3 pt-1">
              <button onClick={() => setReformatConfirm(null)} className="btn btn-secondary py-2 px-4 text-xs font-bold uppercase">{t('modalCancel')}</button>
              <button onClick={() => reformatPaneKeepingFiles(reformatConfirm.which, reformatConfirm.newFmt)} className="btn btn-primary py-2 px-5 text-xs font-bold uppercase">
                {currentLang === 'pt-br' ? 'Recriar' : 'Re-create'}
              </button>
            </div>
          </div>
        </div>
